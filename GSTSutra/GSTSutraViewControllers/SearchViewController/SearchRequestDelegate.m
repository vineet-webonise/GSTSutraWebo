...
@implementation SearchRequestDelegate
...
-(void)searchText:(NSString *)text{
    ...
    requestType = @"SearchAPI";
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", SERVER_API, @"/search_api_by_all"]];
    ...
    [postString appendFormat:@"&%@=%@", @"search_text", [text stringByReplacingOccurrencesOfString:@"&" withString:@"%26" options:NSRegularExpressionSearch range:NSMakeRange(0, [text length])]];
...
-(void)setCommentWithStoryId:(NSString *)storyID withStoryType:(NSString *)storyType withCommentString:(NSString*)comment{
    ...
    [postString appendFormat:@"&%@=%@", @"sid", storyID];
    [postString appendFormat:@"&%@=%@", @"story_type", storyType];
    [postString appendFormat:@"&%@=%@", @"comment", comment];
    [self bodyWithUrlRequest:urlRequest postString:postString];
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    if ([requestType isEqualToString:@"SearchAPI"]) {
        if ([self.delegate respondsToSelector:@selector(searchRequestFailedWithStatus:wihtError:)]) {
            [self.delegate searchRequestFailedWithStatus:@"0" wihtError:error.localizedDescription];
        }
    ...
    @try {
        ...
        if ([requestType isEqualToString:@"SearchAPI"]) {
            if ([[jsonDict objectForKey:@"status"] integerValue]) {
                if ([self.delegate respondsToSelector:@selector(searchRequestSuccessfulWithResult:)]) {
                    
                    [self.delegate searchRequestSuccessfulWithResult:[jsonDict objectForKey:@"result"]];
                }
            } else {
                if ([self.delegate respondsToSelector:@selector(searchRequestFailedWithStatus:wihtError:)]) {
                    [self.delegate searchRequestFailedWithStatus:[jsonDict objectForKey:@"status"] wihtError:[jsonDict objectForKey:@"msg"]];
                }
            }
        }else if ([requestType isEqualToString:@"NewsNotificationAPI"]) {
            if ([[jsonDict objectForKey:@"status"] integerValue]) {
            ...
            if ([[jsonDict objectForKey:@"status"] integerValue]) {
                if ([self.delegate respondsToSelector:@selector(likeNewsRequestSuccessfulWithStatus:)]) {
                    [self.delegate likeNewsRequestSuccessfulWithStatus:[jsonDict objectForKey:@"status"]];
                }
            } else {
                if ([self.delegate respondsToSelector:@selector(likeNewsRequestFailedWithStatus:wihtError:)]) {
            ...
            if ([[jsonDict objectForKey:@"status"] integerValue]) {
                if ([self.delegate respondsToSelector:@selector(setCommentRequestSuccessfulWithStatus:)]) {
                    [self.delegate setCommentRequestSuccessfulWithStatus:[jsonDict objectForKey:@"status"]];
                }
            } else {
                if ([self.delegate respondsToSelector:@selector(setCommentRequestFailedWithStatus:wihtError:)]) {
