...
@interface LongNewsViewController ()<UIGestureRecognizerDelegate,NewRequestDelegate,UITextViewDelegate,UIScrollViewDelegate,MFMessageComposeViewControllerDelegate,MFMailComposeViewControllerDelegate,UIActionSheetDelegate,UIDocumentInteractionControllerDelegate,SFSafariViewControllerDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>{
    
    NSString *postString,*rateValueString,*shareString,*dbBookmarkValue,*dbLikeValue,*editStringID,*dbCommentCountValue,*dbLikeCountValue;
...
@property (weak, nonatomic) IBOutlet UIWebView *longNewsWebView;
...
@implementation LongNewsViewController
...
- (void) receiveNewsNotification:(NSNotification *) notification{
    
    if ([notification.name isEqualToString:@"NotificationNews"]){
        
        NSDictionary* userInfo = notification.userInfo;
        NSString *storyID = (NSString*)userInfo[@"sid"];
        [self stopProgressHUD];
        if ([self checkReachability]) {
            NewRequest *req = [[NewRequest alloc] init];
            req.delegate=self;
            [req getNotificationNewsWithStoryId:storyID withStoryType:@"8"];
        }
        else {
            [self stopProgressHUD];
...
-(void)notificationAPICall{
    ...
    if ([self checkReachability]) {
        ...
        NewRequest *req = [[NewRequest alloc] init];
        req.delegate=self;
        [req getNotificationNewsWithStoryId:appD.sid withStoryType:@"8"];
    }
    else {
        [self stopProgressHUD];
        [self noInternetAlert];
...
-(void)newsNotificationRequestSuccessfulWithResult:(NSArray *)result{
    [self stopProgressHUD];
    self.longViewNewsArray = [result mutableCopy];
...
- (void)viewDidLoad {
    ...
    [self LoadNibTableviewCell];
    [self shareDataSetUp];
}


-(void)LoadNibTableviewCell{
...
-(void)webViewDataAndGesture{
    ...
    @try {
        [self setBookmarkImageWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withBookmarkType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]]];
        ...
        [self setLikeImageWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withBookmarkType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]]];
        ...
        [self updateCommentCountWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withNewsType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]] withCommentCount:[NSString stringWithFormat:@"%lu",[[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"comments"] count]]];
        
        
        [self setCommentCountToLabelWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withNewsType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]]];
        
        [self updateLikeCountWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withNewsType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]] withLikeCount:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"likes_count"]]];
        
        
        [self setLikeCountToLabelWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withNewsType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]]];
        
        self.newsTitleLabel.text = [[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"headline"];
        self.newsTitleLabel.font = [UIFont fontWithName:centuryGothicBold size:titleFont];
        self.newsDateLabel.text = [[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"date"];
        self.ratingView.value = [[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"rating"]floatValue];
        
        [FIRAnalytics logEventWithName:kFIREventSelectContent
                            parameters:@{
                                         kFIRParameterItemID:[NSString stringWithFormat:@"id-%@", [[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]],
                                         kFIRParameterItemName:self.newsTitleLabel.text,
                                         kFIRParameterContentType:@"News"
                                         }];
        
        
        NSString *fullURL = [ImageSERVER_API stringByAppendingString:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"longview"]];
        NSURL *url = [NSURL URLWithString:fullURL];
        NSURLRequest *requestObj = [NSURLRequest requestWithURL:url];
        self.longNewsWebView.backgroundColor = [UIColor whiteColor];
        [self.longNewsWebView loadRequest:requestObj];
        ...
        [self.longNewsWebView.scrollView.panGestureRecognizer requireGestureRecognizerToFail:rightSwipeGesture];
...
-(void)handleRightSwipeGesture:(UISwipeGestureRecognizer*)gestureRecognizer{
    [self storyViewCount];
    if (appD.isFromNotificationAndLogin) {
...
-(void)storyViewCount{
    if ([self checkReachability]) {
        ...
        news.delegate = self;
        [news viewNewsWithStoryId:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] withViewId:@"0"];
    }else {
        [self noInternetAlert];
    }
...
//                          fontSize];
//    [self.longNewsWebView stringByEvaluatingJavaScriptFromString:jsString];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if (navigationType == UIWebViewNavigationTypeLinkClicked){
       ...
       // NSURL *url = request.URL;
        [[UIApplication sharedApplication] openURL:request.URL];;//Handle External URL here
        return NO;
    } else {
        return YES;
    }
...
-(void)rateThisStoryAPICall{
    ...
    if ([self checkReachability]) {
        NewRequest *news = [[NewRequest alloc] init];
        news.delegate = self;
        [news rateNewsWithStoryId:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] withRateValue:rateValueString];
...
//        self.longViewNewsArray = [relatedStoryArray mutableCopy];
//        self.relatedStoryTableView.hidden = YES;
//        [self webViewDataAndGesture];
//       
//    }
    
    {
        ...
        if ([[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] integerValue] == 9) {
...
-(void)postCommentButtonClicked:(UIButton*)sender{
    ...
    if (trimmedString.length > 0 ) {
        if ([self checkReachability]) {
            ...
            [newsReq setCommentWithStoryId:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] withCommentString:postString];
...
-(void)getCommnentsAPICall{
    if ([self checkReachability]) {
        ...
        NewRequest *req = [[NewRequest alloc]init];
        req.delegate = self;
        [req getCommentWithStoryId:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]];
    }
    else {
        [self stopProgressHUD];
        [self noInternetAlert];
...
-(IBAction)editCommentButtonClicked:(UIButton*)sender{
    ...
    editStringID = [NSString stringWithFormat:@"%ld",(long)sender.tag ];
...
-(void)getCommentRequestSuccessfulWithStatus:(NSArray*)result{
    ...
    [self newCommentCountWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withNewsType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]] withCommentCount:[NSString stringWithFormat:@"%lu",(unsigned long)[result count]]];
...
-(void)relatedStoryAPICALL {
    if ([self checkReachability]) {
        ...
        NewRequest *req = [[NewRequest alloc]init];
        req.delegate = self;
        [req newsRelatedWithStoryId:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] andWithStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]];
...
{
//    shareString = [[NSString stringWithFormat:@"%@",ImageSERVER_API] stringByAppendingString:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sharelink"]];
...
- (IBAction)bookmarkButtonClicked:(id)sender {
    ...
    if ([[sender imageForState:UIControlStateNormal] isEqual:[UIImage imageNamed:@"bookmark_n"]]){
        ...
        if ([self checkReachability]) {
            [self startProgressHUD];
            [req bookMarkNewsWithStoryId: [[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] withBookmarkValue:@"1"];
        }
        else {
            [self stopProgressHUD];
            [self noInternetAlert];
        ...
        if ([self checkReachability]) {
            [self startProgressHUD];
            [req bookMarkNewsWithStoryId: [[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] withBookmarkValue:@"0"];
        }
        else {
            [self stopProgressHUD];
            [self noInternetAlert];
...
-(void)bookmarkNewsRequestSuccessfulWithStatus:(NSString *)status{
    ...
    [self updateBookmarkWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withBookmarkType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]]];
    [self stopProgressHUD];
...
- (IBAction)likeButtonClicked:(id)sender {
    ...
    if ([[sender imageForState:UIControlStateNormal] isEqual:[UIImage imageNamed:@"like_n"]]){
        ...
        if ([self checkReachability]) {
            [self startProgressHUD];
            [req likeNewsWithStoryId:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] withLikeValue:@"1"];
        ...
        if ([self checkReachability]) {
            [self startProgressHUD];
            [req likeNewsWithStoryId:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"] withStoryType:[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"] withLikeValue:@"0"];
        }
        else {
            [self stopProgressHUD];
            [self noInternetAlert];
...
-(void)likeNewsRequestSuccessfulWithStatus:(NSString *)status{
    ...
    [self updateLikeWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withBookmarkType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]]];
    [self stopProgressHUD];
    
    [self updateNewLikeCountWithSid:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"sid"]] withNewsType:[NSString stringWithFormat:@"%@",[[self.longViewNewsArray objectAtIndex:self.selectedIndex] objectForKey:@"story_type"]] withLikeCount:[NSString stringWithFormat:@"%@",status]];
