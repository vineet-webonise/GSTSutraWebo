//
//  MostViewedVideoViewController.m
//  GSTSutra
//
...
@interface MostViewedVideoViewController ()<NewRequestDelegate>{
...
@implementation MostViewedVideoViewController
...
-(void)getVideoUrlApiCall{
    if ([self checkReachability]) {
        ...
        req.delegate = self;
        [req getVideoUrlsWithLowerLimit:@"0" withUpperLimit:@"0" videoType:@"mostview"];
    } else {
        [self stopProgressHUD];
        [self noInternetAlert];
    }
...
//            [Newscell.videoThumbnailImageView sd_setImageWithURL:[NSURL URLWithString:[self getYoutubeVideoThumbnail:[[videosArray objectAtIndex:indexPath.row] objectForKey:@"event_code"]]]
